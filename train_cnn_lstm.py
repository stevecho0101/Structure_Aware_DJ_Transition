...
STRIDE  = 64     # step size between windows — 50% overlap so edges are seen twice
...
class SongDataset(Dataset):
    ...
    As songs are different lengths, there must be some overlap between to
    ensure fixed-size inputs"""
    ...
    def __init__(self, csv_paths: list[str], seq_len: int = SEQ_LEN, stride: int = STRIDE):
        ...
        for path in csv_paths:
            ...
            X = data[:, 1:-1].astype(np.float32)   # columns 1..-2 are features (skip time_s and label)
            ...
            for start in range(0, T, stride):       # slide window across song with given stride
                ...
                if end <= T:                         # full window fits — use it directly
                    ...
                    # last window is short — pad with zeros so it's the same size as others
                    pad = end - T
...
class CnnLstm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = x.permute(0, 2, 1)   # CNN expects (batch, channels, time) — swap features and time
        x = self.cnn(x)           # extract local patterns → (batch, 64, seq_len)
        x = x.permute(0, 2, 1)   # LSTM expects (batch, time, features) — swap back
        x, _ = self.lstm(x)       # model long-range structure → (batch, seq_len, 256)
        x = self.dropout(x)       # regularize before final prediction
        return self.head(x)       # output one score per class per second → (batch, seq_len, 4)
...
def train():
    ...
    model     = CnnLstm().to(device)        # instantiate model and move to compute device
...
def predict(mp3_path: str) -> np.ndarray:
    ...
    """
    model_path = os.path.join(BASE, "cnn_lstm_best.pt")  # always use the saved model
    ...
    X_t = torch.from_numpy(X).unsqueeze(0).to(device)            # add batch dimension → (1, T, 39)
    X_t = (X_t - g_mean) / g_std                                 # apply same normalization used during training

    with torch.no_grad():                                          # no gradient needed for inference
        logits = model(X_t)                                        # forward pass → (1, T, 4)
        preds = logits.squeeze(0).argmax(dim=-1).cpu().numpy()    # pick best class per second → (T,)
    ...
    # Force the last contiguous segment to outro — songs should always end with outro
    last_label = preds[-1]
    i = len(preds) - 1
